...
function getBudgetSettingsRedirectTarget(type: LedgerEntryTypeValue) {
  return `/ledger/settings/budgets?type=${type}`;
}
...
async function redirectWithToast(request: Request, type: "success" | "error", message: string, selectedType: LedgerEntryTypeValue = "EXPENSE") {
  ...
  return redirect(getBudgetSettingsRedirectTarget(selectedType), {
    headers: {
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (categoryResponse) {
    ...
    return redirectWithToast(request, "success", getCategorySuccessMessage(payload, formData), selectedType);
  }
  ...
  for (const type of LEDGER_BUDGET_TYPE_ORDER) {
    ...
    if (!Number.isFinite(totalBudget) || totalBudget < 0) {
      return redirectWithToast(request, "error", `${getBudgetSectionMeta(type).totalLabel}은 0 이상의 숫자로 입력해 주세요.`, selectedType);
    }

    const weekCarryMode = parseWeekCarryMode(formData.get(`weekCarryMode_${type}`));
    if (!weekCarryMode) {
      return redirectWithToast(request, "error", `${getTypeLabel(type)} 주별 이월 방식을 다시 확인해 주세요.`, selectedType);
    }
  ...
  return redirectWithToast(request, "success", "기본 예산 상세 설정을 저장했습니다.", selectedType);
};

export default function LedgerBudgetSettingsPage() {
  const { selectedType, budgetPeriodLabel, plans, categoriesByType } = useLoaderData<typeof loader>();
  ...
  const actionUrl = getBudgetSettingsRedirectTarget(selectedBudgetType);
