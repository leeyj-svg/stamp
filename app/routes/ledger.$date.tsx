...
function getCurrentWeekDisplayText(type: LedgerEntryTypeValue) {
  if (type === "EXPENSE") {
    return "이번 주 남은 예산";
  }
...
async function redirectWithLedgerToast(
  ...
  return redirect(buildLedgerDateLink(dateToken, monthToken, filter, showCurrentWeekBudget, selectedCategoryIds), {
    headers: {
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const [entries, categories, monthCategoryRows] = await Promise.all([
...
export default function LedgerDatePage() {
  ...
  const resultAmount = useMemo(() => {
    if (selectedFilter === "INCOME") {
      return filteredSummary.income;
    }

    if (selectedFilter === "EXPENSE") {
      return -filteredSummary.expense;
    }

    if (selectedFilter === "SAVING") {
      return -filteredSummary.saving;
    }

    return filteredSummary.income - filteredSummary.expense - filteredSummary.saving;
  }, [filteredSummary, selectedFilter]);
    ...
    <>
      <div className="min-h-screen bg-white">
        <div className="border-b bg-white px-2 py-3">
          <div className="flex items-center justify-between gap-2">
            <Button asChild variant="ghost" size="icon" className="h-10 w-10 rounded-full text-slate-700">
              <Link
                to={`/ledger?month=${calendarMonthToken}${selectedFilter !== "ALL" ? `&type=${selectedFilter}` : ""}${
                  buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)
                    ? `&${buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)}`
                    : ""
                }`}
              >
            ...
            <div className="flex items-center gap-1">
              <Button asChild variant="ghost" size="icon" className="h-10 w-10 rounded-full text-slate-700">
                <Link to={buildLedgerDateLink(prevDateToken, calendarMonthToken, selectedFilter, showCurrentWeekBudget, selectedCategoryIds)}>
              ...
              <Button asChild variant="ghost" size="icon" className="h-10 w-10 rounded-full text-slate-700">
                <Link to={buildLedgerDateLink(nextDateToken, calendarMonthToken, selectedFilter, showCurrentWeekBudget, selectedCategoryIds)}>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem asChild>
                  <Link
                    to={`/ledger/stats?month=${calendarMonthToken}${selectedFilter !== "ALL" ? `&type=${selectedFilter}` : ""}${
                      buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)
                        ? `&${buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)}`
                        : ""
                    }`}
                ...
                <DropdownMenuItem asChild>
                  <Link
                    to={`/ledger/new?date=${dateToken}${selectedFilter !== "ALL" ? `&type=${selectedFilter}` : ""}${
                      buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)
                        ? `&${buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)}`
                        : ""
                    }`}
        ...
        <div className="grid grid-cols-3 border-b bg-white">
          <Link
            to={buildLedgerDateLink(dateToken, calendarMonthToken, toggleEntryFilter(selectedFilter, "INCOME"), showCurrentWeekBudget)}
            className={cn(
              "py-3 text-center transition-colors",
              selectedFilter === "INCOME" ? "bg-sky-50" : "hover:bg-slate-50",
            )}
          >
          ...
          <Link
            ...
            className={cn(
              "py-3 text-center transition-colors",
              selectedFilter === "EXPENSE" ? "bg-rose-50" : "hover:bg-slate-50",
            )}
          >
          ...
          <Link
            ...
            className={cn(
              "py-3 text-center transition-colors",
              selectedFilter === "SAVING" ? "bg-emerald-50" : "hover:bg-slate-50",
            )}
          >
        ...
        <div className="pb-24">
            ...
            <div className="border-y border-slate-200 bg-white">
              {filteredEntries.map((entry, index) => {
                ...
                return (
                    <Link
                      key={entry.id}
                      to={`/ledger/entries/${entry.id}/edit?month=${calendarMonthToken}${selectedFilter !== "ALL" ? `&type=${selectedFilter}` : ""}${
                        buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)
                          ? `&${buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)}`
                          : ""
                      }`}
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0 flex-1 pl-1 pt-1">
                        {memoText ? (
                          <p className="truncate text-[0.78rem] font-semibold leading-tight text-slate-700">{memoText}</p>
                        ) : null}
                        {detailText ? (
                          <p className="mt-1 truncate text-[0.68rem] leading-tight text-slate-400">{detailText}</p>
                        ) : null}
                      </div>
          ...
          <div className="border-t border-slate-200 bg-white px-4 py-3">
            <div className="flex items-center justify-end text-sm font-semibold">
              <span className={cn("whitespace-nowrap", resultClassName)}>
                {formatLedgerAmount(resultAmount)}
              </span>
            </div>
          </div>
        </div>
      ...
      <Button
        asChild
        size="icon"
        className="fixed bottom-5 right-5 z-30 h-14 w-14 rounded-full bg-slate-500 shadow-lg hover:bg-slate-600"
      >
        <Link
          to={`/ledger/new?date=${dateToken}${selectedFilter !== "ALL" ? `&type=${selectedFilter}` : ""}${
            buildBudgetQuery(showCurrentWeekBudget, selectedCategoryIds)
